...
The app is **React Native + Expo (iOS-first)**, structured in layers that each have one job. The **adapter layer** (`lib/api/*`) wraps each external data source in a typed contract; the rest of the app sees `Zone[]`, `Route[]`, and `Place[]` rather than the idiosyncratic JSON each provider returns. The **scoring layer** (`lib/scoring.ts`) is a pure function: given a candidate route and a set of zones, it returns a deterministic score. There is no I/O in scoring, no time-dependent surprise — the same inputs always produce the same routing decision, so safety choices are reproducible and inspectable. The **screen layer** (`app/*`) renders the result. Community reports remain local-first in AsyncStorage and sync through Fresh Greens repositories when Supabase is configured. Only `lib/supabase/` may import the Supabase SDK, access its configured client, or construct Supabase service URLs; screens and hooks stay behind product-owned adapters and repositories.
...
- **Community reports are local-first and Supabase-backed when configured.** AsyncStorage keeps the immediate on-device experience and retry queue; the Supabase repositories provide cross-device reads, authenticated inserts, soft deletion, roles, and moderation. The remaining release gate is validating the deployed schema and RLS behavior across two real devices.
- **Apple Sign-In is wired to Supabase identity in code.** A configured build links an anonymous backend session to Apple or restores the existing permanent Supabase user. Shipping still requires the Apple capability and Supabase Apple provider to be configured for the permanent bundle identifier, followed by real-iPhone verification. Push Notifications have their own Apple entitlement and release setup.

What IS shipped, end-to-end: zone-aware routing across the three public data sources, the daylight-aware route geometry via SunCalc, the route-preview card with safety chips and an all-clear state, multi-row community/category browsing, the en-route experience with live turn-by-turn (Mapbox Directions banner instructions + lane guidance, OSRM fallback, offline route cache) and the side-button column, the `/pulled-over` 5-phase safety state machine with audio recording and trusted-contact wiring, real-time weather via Open-Meteo, scheduled departure notifications, alternate-route comparison, refuel reminders (local notifications), in-app community reporting with optional camera photo capture (`expo-image-picker`), the iOS grouped-settings register across all six settings pages, and Connect-Calendar (read-only: upcoming located events surface as one-tap destinations in `/search`, verified on-device). Three months of working argument; the gaps above are intentional, not unfinished.
