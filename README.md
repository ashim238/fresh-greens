...
- **Community reports are device-local — no backend yet.** Reports are stored in `AsyncStorage` and stamped with the real signed-in user's id (or anonymous for anonymous categories), but there's no server, so reports don't sync across devices: "the community" is functionally per-phone. The scoring pipeline treats reports as a first-class signal regardless; the v2 path is a Supabase/Firebase backend behind the existing adapter contract.
