...
### Hierarchy
- **Brand Display Large** (DM Serif Display Regular, 34/42, 0 tracking): Emotional hero moments — the /emergency heading ("Need help?"), sign-out farewell, and arrival/trip-summary destination. The warm serif register break.
- **Brand Display** (DM Serif Display Regular, 28/36, 0 tracking): In-flow emotional moments — pulled-over contact reassurance, the "You're not alone" lifeline, roadside thank-you, report confirmation.
- **Brand Display Small** (DM Serif Display Regular, 22/30, 0 tracking): Reserved. Defined for a smaller-scale emotional moment but currently unused — DM Serif is never applied to settings or navigation (see `.impeccable/design.json`), so the "Hey there," greeting stays Franklin.
...
**The Hero-Title Rule.** A full-screen title does **not** by itself earn the serif — the register break follows the *emotion*, not the screen weight. The get-started welcome ("Get started") and login prompt ("Welcome back") own their screens but are wayfinding, so they stay in **Large Title — Emphasized** (Franklin Bold). Only the sign-out farewell — a genuine goodbye — crosses into Brand Display Large among the full-screen titles.

**The Brand-Voice Rule.** DM Serif Display is reserved to six emotional moments (source of truth: `.impeccable/design.json`) — the /emergency heading, pulled-over reassurance, sign-out farewell, arrival, roadside thank-you, and report confirmation. Everything else — navigation, section headers, **settings (including the "Hey there," greeting)**, and utility titles — stays in Franklin. The serif/sans boundary is the boundary between warmth and structure. When in doubt, it's Franklin: the serif is rationed the same way the reserved colors are.
