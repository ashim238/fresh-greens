...
- **C8 — emergency/SOS control with trusted-contact ↔ 911 choice** → ✅ closed (PR C8). New `app/emergency.tsx` (pushed full-screen, reached from a prominent navy SOS bar on `/safety`): **tap** → 3s cancelable countdown → real `tel:` to the trusted contact; **hold** → the button fills red → a **guarded** 911 confirm (an explicit "Call 911" button the user taps deliberately — the hold never auto-dials). Reduce-motion gated, reserved-color compliant (navy ground, red only during the 911 escalation), with an explicit non-gesture 911 path for VoiceOver/motor-impaired users. This also lifts **C9** (the trusted-contact graph's two coping behaviors) from partial toward complete — the institutional-help half (911) now exists alongside the community-call half. _The "guard" is that 911 is never auto-triggered, only via a deliberate button tap._
- **C12 — post-trip summary with inference-validation + real persistence** → ✅ closed (PRs C12a/b/c). (a) The `/trip-summary` recap is reachable — the `/en-route` arrival terminal state pushes it (was unreachable). (b) **Inference-validation loop:** `/en-route` accumulates the caution/avoid zones the trip actually drove through (incl. police point-zones); `/trip-summary` lists each for confirm/dismiss, and a confirmation creates a community report at the zone's `zoneAnchor` point — so lived validation feeds future routing via `getCommunityReportsAsZones → scoring` (the countermapping loop). (c) **"Set as default" persists:** marks the destination a "regular" (new `regular-destinations` adapter + `useRegularDestinations` hook), which flips `home.tsx`'s long-hardcoded `isRegularDestination` from `false` to a real `isRegularLocation` check — the recurring-destination underline now works, and it's the first frequency signal toward the C15 adaptive-personalization spine. Cleared on sign-out for identity parity.
