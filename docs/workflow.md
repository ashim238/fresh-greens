...
| `{{DESIGN_SPECIMEN}}` | **None — retired 2026-06-17.** The hand-mirrored `fresh-greens-specimen/` HTML showcase + its `check:specimen` drift-check are gone (a hand-mirror that drifts isn't worth the chore). The design system now lives in three derived-from-`theme/` forms, no hand-sync step: `theme/` (app source of truth) → `.impeccable/design.json` (rich machine catalog, generated by the impeccable skill) → [`DESIGN.md`](../DESIGN.md) (human-readable summary). So **§4 has no specimen-mirror step.** |
