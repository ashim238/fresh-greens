...
These wrap the per-PR rhythm above rather than being a step in it.

- **task-observer** (a.k.a. "one skill to rule them all") — invoke at the **start of any task-oriented session** (this mirrors the rule in the global `~/.claude/CLAUDE.md`; it's restated here so the project rhythm and the global rule stay cross-referenced). It captures skill-improvement signal — user corrections, workflow insights, methodology worth preserving — as you work. When you load any skill, also check its OPEN observations and apply them. It changes nothing about the dev flow; it's a passive capture layer.
