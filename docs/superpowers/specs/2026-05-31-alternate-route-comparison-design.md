...
- **Alternates availability (source-agnostic)** — the routing ladder is **Mapbox Directions (primary) → OSRM (fallback) → cache → mock**; both Mapbox and OSRM return `alternatives` (usually 1–2). The feature operates on `pickWinner`'s `RankedRoute[]` regardless of which tier produced them, and the condition-chip proximity math holds for both providers' dense geojson geometry (and degrades gracefully for cache/mock single routes). Decision for the zero-alternates case: **keep the FAB and show the single recommended row** in the sheet (labeled "Safest route" with its condition chips). The FAB stays put (hiding it would unbalance the ETA-row bracket layout), and a one-row sheet is still honest/useful (your route's conditions at a glance) rather than a dead-end. No special single-route screen.
