...
- **`menu-home.svg` re-export — glyph-only.** The current `assets/illustrations/menu-home.svg` exports the full Figma `1133:13222` Button component (64×64 white pill + shadow + inner 24×24 hamburger). To slot cleanly inside `FloatingActionButton` on /home's hamburger button (which has its own pill chrome), the SVG needs a re-export at Figma node `1102:4155` (just the inner Icon, 24×24 viewBox). Currently /home uses the as-shipped SVG inside a plain Pressable to avoid double-chrome; when the glyph-only export lands, re-wrap in `FloatingActionButton` for the matching avatar-pill register.
...
- **/pulled-over bulk Ionicons migration — partial.** `chore/pulled-over-bulk-svg-swap` resolved the read-aloud audio-control affordance: `volume-high-outline` / `stop-circle-outline` → Phosphor `SpeakerHigh` / `Stop`, aligning /pulled-over's audio register with /recordings's Phosphor Play/Pause/Microphone for the same recording data. Still outstanding (canonical SVG exports not yet on disk): (a) Call button glyph (currently `Ionicons call`), (b) Text button glyph (currently `Ionicons chatbubble`), (c) guidance-phase Continue chevron + review-phase nav chevrons (currently `Ionicons chevron-forward/back` — matches the rest of the project's de-facto chevron register, not blocking), (d) the four 120pt Review sub-view header illustrations (`car-outline` / `card-outline` / `chatbubble-ellipses-outline` / `library-outline` for What to Do/Have/Say/Know) — these are the **highest-leverage punch-list item**, hero-sized stand-ins that degrade fidelity disproportionately, and need custom Figma exports the same way Officer/Trooper got theirs in `feat/illustrations`.
- **/home Report button parity with /en-route.** ✅ Resolved in `chore/figma-fidelity-audit-4`. /home now consumes `FloatingActionButton size="56"` + the canonical `sidebtn-report.svg` — identical chrome + glyph to /en-route's side-column Report. The bespoke `reportBtn` style block and Ionicons `alert-circle` retired.
- **/safety modal header shield → canonical navy duotone.** ✅ Resolved in `chore/figma-fidelity-audit-4`. /safety's header now uses `sidebtn-safety.svg` (matching /en-route's FAB and /menu's Safety row). Three surfaces, one glyph, consistent "safety affordance" register per reserved-color rule #6.
- **EnRouteCarMarker first-paint snapshot fix.** ✅ Resolved in `chore/figma-fidelity-audit-4`. The marker now starts with `tracksViewChanges={true}` and flips to `false` 50ms after mount, giving react-native-svg's async path resolution time to land before MapKit caches the snapshot. Resolves the "car disappears after first paint until heading rotates" bug.
