...
| 🔴 | **Long-press on own community pin doesn't trigger Remove Alert** | `/home` | Surfaced during device-smoke cluster 1 retest (2026-06-21). `handleLongPress` hit-test (`degPerPxLat * 30`) likely doesn't match the actual marker render size — falls through to "Save as home" Alert instead. Verify the marker radius (orange-eye pin actual SVG size) against the hit-radius computation. |
