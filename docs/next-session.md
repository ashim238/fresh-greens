...
| 🟣 | **Roadside tow-truck flow — call-first, less sheet stacking** | `/roadside` → `/search` | Today "Search nearby tow services" pushes full `/search` on top of the `/roadside` modal; results route to `/home` (navigation metaphor). When stranded, job-to-be-done is dispatch (call), not re-route. Spec: inline nearby-tow list with `tel:` primary actions vs embedded sheet vs Maps handoff; avoid layer-on-layer modals. Grill-me started 2026-06-25. |
| 🟣 | Why does the app jump to Apple Maps for nearby resources? | `/roadside` tow search, others | Strategic UX: delegate to system Maps vs in-app surface. Tow / fuel / repair findability. Fold into tow-truck item above when speccing. |
