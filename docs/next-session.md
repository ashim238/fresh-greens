...
**Still open — WCAG 1.4.1 severity-chip color-only signaling (one-off, small):** the report sub-tag chips ([`app/report.tsx`](../app/report.tsx) `severityChipStyles`, ~531) encode avoid/caution via a red/orange border + 8% fill only — add a subtle non-color cue (e.g. a small `WarningDiamond` on flagged chips, no severity words, keep it calm). **Correction (2026-06-26):** an earlier note here batched this with "the daylight gradient" as a shared color-independence pass — that was wrong. Grounding showed the daylight polyline **already** ships the non-color cue (`DAYLIGHT_DASH_PATTERN` solid/dashed/dotted from `lib/daylight.ts` + the `DaylightRouteLegend`); PRODUCT.md's stale "known gap #1" had said otherwise and is now fixed. So this reduces to just the severity chips. (Lesson, again: verify against `lib/daylight.ts`, not the stale doc.)
>
