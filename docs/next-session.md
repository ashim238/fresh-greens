...
**Still open — color-independence a11y pass (WCAG 1.4.1), batched:** severity-chip color-only signaling ([`app/report.tsx`](../app/report.tsx) `severityChipStyles`, ~531) **+** the already-tracked daylight-gradient color-only polyline (PRODUCT.md known gaps). Do both in one dedicated pass — subtle non-color cue on the chips (e.g. a small `WarningDiamond` on flagged chips, no severity words — keep it calm), dash/width pattern + inline legend on the gradient. Rationale: same WCAG class; "color independence across the app" is a cleaner, more thesis-legible work item than one-off chip tweaks.
>
