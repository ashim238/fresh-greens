...
- **/pulled-over bulk Ionicons migration** queued for the next bulk-SVG pass. `app/pulled-over.tsx` is the last screen with ≥4 Ionicons stand-ins: read-aloud (`volume-high-outline` / `stop-circle-outline`), Continue chevron (`chevron-forward`), Call (`call`), and the four Review sub-view header illustrations (`car-outline` / `card-outline` / `library-outline`, etc. at size 120 — large enough that the Ionicons fidelity gap is most visible here). The custom Figma illustrations for the Review sub-views (Officer, Trooper, What to Do/Have/Say/Know) are already covered separately under the audit queue, but the smaller affordances (Call icon, read-aloud volume, chevron) need their canonical SVG exports too.
