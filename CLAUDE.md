...
- **Asset format default: SVG.** When Figma provides a vector source, default to SVG (`react-native-svg-transformer` resolves the import to a real component, scales clean at every density). PNG is the exception, reserved for photographic content. Past mistakes — Vic, the three onboarding panels, the get-started cars — shipped as 1x PNGs and went visibly blurry on retina at any size above ~48pt. The threshold worth remembering: **if the asset is over ~48pt in either dimension and the Figma layer is vector, export as SVG.** PNG sneaks past code review easily because it works on simulator at @1x; the bug only surfaces on real hardware.
...
- ~~Welcome illustrations: clouds + wind.~~ ✅ Shipped in `feat/illustrations`, then re-architected as individual animated layers in `feat/welcome-cloud-animation`. Welcome renders 14 absolutely-positioned SVG layers from Figma 825:3163 inside a 390×846 bottom-centered backdrop scene (Vic was the last PNG holdout — swapped to SVG in `fix/onboarding-illustrations-svg` after the same density issue showed up on the onboarding panels). The 7 cloud + 3 wind elements wrap in a `Drift` helper that runs `Animated.loop` translateX oscillation with `Easing.inOut(Easing.sin)` + `useNativeDriver: true`; per-element durations spread across 2.8s–5.8s naturally desync them. Content (title, terms, buttons) overlays via SafeAreaView.
