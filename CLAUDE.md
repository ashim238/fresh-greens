...
- ~~Welcome illustrations: clouds + wind.~~ ✅ Shipped in `feat/illustrations`, then re-architected as individual animated layers in `feat/welcome-cloud-animation`. Welcome renders 14 absolutely-positioned layers (Vic PNG + 13 SVGs from Figma 825:3163) inside a 390×846 bottom-centered backdrop scene. The 7 cloud + 3 wind elements wrap in a `Drift` helper that runs `Animated.loop` translateX oscillation with `Easing.inOut(Easing.sin)` + `useNativeDriver: true`; per-element durations spread across 2.8s–5.8s naturally desync them. Content (title, terms, buttons) overlays via SafeAreaView.
...
- ~~Custom map markers (saved home, community reports).~~ ✅ Shipped in two PRs. `feat/map-markers-and-edge-indicators` set up the foundation: `lib/api/saved-places.ts` adapter + `useSavedPlaces` hook + `MapMarker` component (circular pip, used for saved home with a Phosphor House glyph; saved via long-press on the map → `Alert.alert` confirm). `feat/map-marker-icons-from-figma` then layered the Figma-faithful **`LandmarkMarker`** component for community reports — three states from Figma `1044:2667` (black-owned: black pin + brand-green storefront; local-business: gray pin + white menu glyph; report: orange pin + per-category Ionicons + a Figma SVG eye for felt-unsafe). `Zone.reportCategoryId` (optional) carries the category id off the community-reports adapter so the marker variant + glyph reflect the submission. **Trusted-friend marker is queued** — needs a saved location attached to the contact (current `lib/api/trusted-contact.ts` has no lat/lng), one-feature follow-up.
