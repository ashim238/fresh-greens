...
- Safety Settings (`/safety-settings`) — pushed from /menu's Safety row. Hosts safety-flow preferences. v1 has a single Trusted Contact row that re-uses `/trusted-contact-setup?from=settings`. Future safety prefs slot in as additional rows.
