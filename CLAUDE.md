...
- **`local-business` (gray) variant cleanup** queued for the next Figma fidelity audit. The 4-variant LandmarkMarker from Figma `1044:2667` includes a gray "Local Business" pin that's currently unreachable — `variantForCategoryId` no longer routes anything to it (home was briefly mapped to it in `feat/community-report-mechanic` but moved to `positive`/green for visual consistency with the otherwise vibrant black/green/orange system). The Figma component still has the variant, the pin/bg/glyph SVGs still exist in `assets/illustrations/`, and the type union still includes it. Audit decision: either (a) delete the variant entirely from code + type + SVGs (cleaner, but drifts from the Figma source of truth) or (b) keep the dormant variant and document it as reserved-but-unused. If kept, update LandmarkMarker's docstring to reflect that it's not currently routable. Either way, decide explicitly so it's not lingering ambiguity.
