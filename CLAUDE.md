...
- ~~Custom map markers (saved home, community reports).~~ ✅ Shipped in `feat/map-markers-and-edge-indicators` — `lib/api/saved-places.ts` adapter (one-home-at-a-time invariant; landmarks supported but not yet UX-exposed) + `useSavedPlaces` hook + `MapMarker` component (Phosphor-glyph in a circular pip). Saved home is set via long-press on the map → `Alert.alert` confirm. Community reports render as custom Markers (Megaphone glyph tinted by zone severity); the previous Circle rendering is retired. **Trusted-friend marker is queued** — needs a saved location attached to the contact (current `lib/api/trusted-contact.ts` has no lat/lng), one-feature follow-up.
