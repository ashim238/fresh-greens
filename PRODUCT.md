...
WCAG 2.1 AA is the target. The app already invests in Dynamic Type (`dynamicType()` wrapper), VoiceOver labels + hints, 44pt tap targets, and Reduce Motion gating in several places. Known gaps tracked for this work: (1) **WCAG 1.4.1 daylight-gradient color-only signaling** — the route polyline encodes daylight via orange→mauve→indigo with no non-color cue, unreadable to colorblind users; needs a redundant non-color channel (dash/width pattern) plus an inline legend. (2) Broader Dynamic Type coverage and AX5 truncation testing. (3) Reduce Motion gating on the home browse carousel snap. Accessibility is treated as a thesis strength to foreground, not a checkbox — it maps directly to "whose needs the design accounts for."
